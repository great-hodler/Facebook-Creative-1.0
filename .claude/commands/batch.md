...
**You produce 10 complete creatives per run. Each creative includes: brief, copy, image prompt, image-prompt.json, and approved.md. Images are generated autonomously by calling `python3 generate_batch_images.py` via the Bash tool — you run this yourself, no human step required.**
...
**MANDATORY: Start every kie.ai prompt with the visual style prefix for this format:**
...
## kie.ai API Request — 1:1 (Square Feed)
{
  "prompt": "...",
  "aspect_ratio": "1:1",
  "output_format": "jpg"
...
## kie.ai API Request — 9:16 (Stories / Reels)
{
  "prompt": "...",
  "aspect_ratio": "9:16",
  "output_format": "jpg"
...
| 1:1 Image | image-1x1-final.jpg |
| 9:16 Image | image-9x16-final.jpg |
...
- [ ] 1:1 image generated (auto-run via generate_batch_images.py)
- [ ] 9:16 image generated (auto-run via generate_batch_images.py)
...
## STEP 3 — RUN IMAGE GENERATION (AUTONOMOUS)

After all 10 creatives have their `image-prompt.json` files written, use the Bash tool to run:

```bash
cd /home/user/Facebook-Creative-1.0 && python3 generate_batch_images.py
```

**This is mandatory — you must execute this command yourself using the Bash tool. Do NOT ask the user to run it.**

The script reads all `output/*/image-prompt.json` files and calls the kie.ai Nano Banana Pro API to generate images. It polls for completion and downloads the results as `image-1x1-final.jpg` and `image-9x16-final.jpg` in each creative folder.

- Images already generated are skipped (safe to re-run)
- On failure, the script retries 3× then logs and continues — never aborts the batch
- Estimated cost: ~$0.40 per batch (20 images × $0.02)

After the script completes, commit and push everything including the images:

```bash
git add output/ && git commit -m "feat: Batch [date]-[nn] — 10 creatives + 20 images" && git push -u origin claude/facebook-ads-creative-system-pVJG0
```
...
5. **Never commit `.env`** — it contains API keys and must never appear in git.
...
12. **Every kie.ai prompt must start with the visual style prefix for its format.** No exceptions.
13. **Every batch must satisfy the visual treatment diversity requirements** (F7, F2/F9, F3/F8, F5, F4 each represented).
14. **Run `python3 generate_batch_images.py` yourself via the Bash tool.** Never ask the user to open a terminal.
